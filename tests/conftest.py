...
# накатывает миграции на тестовую бд
@pytest.fixture(scope="session", autouse=True)
async def run_migrations():
    os.system("alembic init migrations")
    os.system('alembic revision --autogenerate -m "test runnig migrations"')
...
@pytest.fixture
async def create_user_from_database(asyncpg_pool):

    async def create_user_from_database_by_uuid(user_id:str, name:str,surname:str,email:str, is_active:bool):
        async with asyncpg_pool.acquire() as connection:
            return await connection.execute("""INSERT INTO users VALUES ($1, $2, $3, $4, $5)""", user_id, name, surname,email, is_active)
        
    return create_user_from_database_by_uuid