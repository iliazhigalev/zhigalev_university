...
async def _update_user(body:UpdateUserRequest, user_id:UUID,db) -> Union[UUID, None]:
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
            updated_user_id = await user_dal.update_user( user_id = user_id, **body.dict(exclude_none=True) )
...
async def _delete_user(user_id, db):
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
            delete_user =await user_dal.delete_user(user_id=user_id,)
            return delete_user
...
async def update_user_by_id(
    ...
    if body.dict(exclude_none=True)=={}:
        raise HTTPException(status_code=422,detail="At least one parameter for user update info")
    user = await _get_user_by_id(user_id, db)
    if user is None:
        raise HTTPException(status_code=404, detail=f"User with id {user_id} not found")
    update_user_id = await _update_user(body=body, db=db, user_id=user_id)
